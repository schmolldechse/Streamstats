...
{
    public class SubscriptionGroupBox : GroupBox
    {
        ...
        public SubscriptionGroupBox(Subscription subscription) 
        {
            if (subscription == null)
            {
                throw new ArgumentException("Subscription is null or invalid");
            }
            ...
            {
                if (this.timeAgo_TextBlock != null) this.timeAgo_TextBlock.Text = this.timeDifference(this._subscription.activity.createdAt);
            };
            this.timer.Start();
            ...
            icon.Source = new BitmapImage(new Uri("../../Images/Subscriber.ico", UriKind.RelativeOrAbsolute));
            icon.Height = 20;
            icon.Width = 20;
            ...
            username.Text = _subscription.user.username;
            username.Foreground = new SolidColorBrush(Color.FromRgb(243, 175, 28));
            ...
            amount_Border.BorderBrush = new SolidColorBrush(this.gray);
            amount_Border.Background = new SolidColorBrush(this.gray);
            amount_Border.CornerRadius = new CornerRadius(10);

            TextBlock amount_Inner = new TextBlock();
            amount_Inner.Text = _subscription.amount + " Months";
            ...
            // Top left | tier
            TextBlock tier_Outer = new TextBlock();
            tier_Outer.Margin = new Thickness(5, 0, 0, 0);

            Border tier_Border = new Border();
            tier_Border.BorderBrush = new SolidColorBrush(this.gray);
            tier_Border.Background = new SolidColorBrush(this.gray);
            tier_Border.CornerRadius = new CornerRadius(10);

            TextBlock tier_Inner = new TextBlock();
            tier_Inner.Text = _subscription.tier.ToUpper();
            tier_Inner.Margin = new Thickness(10, 4, 10, 4);
            tier_Inner.Foreground = Brushes.FloralWhite;

            tier_Border.Child = tier_Inner;
            tier_Outer.Inlines.Add(tier_Border);

            // Add to top left
            topLeft.Children.Add(icon);
            topLeft.Children.Add(username);
            topLeft.Children.Add(amount_Outer);
            topLeft.Children.Add(tier_Outer);
            ...
            timeAgo.Text = timeDifference(this._subscription.activity.createdAt);
            ...
            if (this._subscription.message.Length > 0)
            {
                ...
                foreach (string segment in this._subscription.message.Split(' '))
                {
            ...
            Task.Run(async () =>
                            {
                                ...
                                using HttpResponseMessage responseMessage = await App.httpClient.PostAsync(string.Format(STREAMELEMENTS_REPLAY_API, App.se_service.channelId, this._subscription.activity.id), jsonContent);
